...
def load_data(use_local=True):
    ...
    if config is None:
        print(
            "Error: config.json not found. Please run setup.py in the root directory."
        )
        df = pd.DataFrame()
        return
...
def get_map_image_url(map_name):
    ...
    for ext in [".jpg", ".png"]:
        ...
        if os.path.exists(os.path.join("assets", "maps", f"{cleaned_name}{ext}")):
            return asset_path
    return "/assets/maps/default.png"
...
def get_hero_image_url(hero_name):
    ...
    for name in potential_names:
        ...
        for ext in [".png", ".jpg", ".jpeg"]:
            ...
            if os.path.exists(os.path.join("assets", "heroes", f"{name}{ext}")):
                return asset_path
    return "/assets/heroes/default_hero.png"
