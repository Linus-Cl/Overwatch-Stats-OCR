...
def load_data(use_local=True):
    global df, config
    if config is None:
        print("Error: config.json not found. Please run setup.py in the root directory.")
        df = pd.DataFrame()
        return

    if use_local and os.path.exists("local.xlsx"):
        try:
        ...
        try:
            ...
            if "df" not in globals() or df.empty: df = pd.DataFrame()

    if not df.empty:
        df.columns = df.columns.str.strip()
        if "Date" in df.columns: df["Date"] = pd.to_datetime(df["Date"], errors="coerce")
        if "Match ID" not in df.columns: df["Match ID"] = range(len(df), 0, -1)
...
app.layout = dbc.Container(
    [
        ...
        dbc.Row([
            dbc.Col(html.Img(src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/55/Overwatch_circle_logo.svg/1024px-Overwatch_circle_logo.svg.png", height="50px"), width="auto"),
            dbc.Col(html.H1("Overwatch Statistics", className="my-4"), width=True),
            dbc.Col(dbc.Button("Update Data from Cloud", id="update-data-button", color="primary", className="mt-4"), width="auto"),
        ...
        dbc.Row([
                ...
                dbc.Card([
                    ...
                    dbc.CardBody([
                        ...
                        dcc.Dropdown(id="player-dropdown",
                            options=[{"label": p, "value": p} for p in (config["known_players"] if config else [])],
                            value=config["known_players"][0] if config and config["known_players"] else None,
                            clearable=False, className="mb-3"),
                        dbc.Label("Select Season (overrides Year/Month):"),
                        dcc.Dropdown(id="season-dropdown", placeholder="(no selection)", className="mb-3", clearable=True),
                        dbc.Label("Select Year:"),
                        dcc.Dropdown(id="year-dropdown", placeholder="(no selection)", className="mb-3", clearable=True),
                        dbc.Label("Select Month:"),
                        dcc.Dropdown(id="month-dropdown", placeholder="(no selection)", className="mb-3", clearable=True),
                        dbc.Label("Minimum Games Played:"),
                        dcc.Slider(id="min-games-slider", min=1, max=100, step=None, value=5,
                            marks={1: "1", 5: "5", 10: "10", 25: "25", 50: "50", 75: "75", 100: "100"},
            ...
            dbc.Col([
                dbc.Tabs([
                    dbc.Tab(label="Map & Mode Stats", tab_id="tab-map", children=[
                        dbc.Row([
                            dbc.Col(dcc.Dropdown(id="map-stat-type", value="winrate", clearable=False, style={"width": "100%", "marginBottom": "20px"},
                                options=[
                            ...
                            dbc.Col(html.Div(dbc.Switch(id="map-view-type", label="Detailed View", value=False, className="mt-1"), id="map-view-type-container", style={"marginBottom": "20px"}), width=4, className="d-flex align-items-center"),
                        ]),
                        html.Div(id="map-stat-container"),
                    ]),
                    dbc.Tab(label="Hero Stats", tab_id="tab-hero", children=[
                        dcc.Dropdown(id="hero-stat-type", value="winrate", clearable=False, style={"width": "300px", "marginBottom": "20px"},
                            options=[
                    ...
                    dbc.Tab(label="Role Stats", tab_id="tab-role", children=[
                        dcc.Dropdown(id="role-stat-type", value="winrate", clearable=False, style={"width": "300px", "marginBottom": "20px"},
                            options=[
                    ...
                    dbc.Tab(dcc.Graph(id="performance-heatmap"), label="Performance Heatmap", tab_id="tab-heatmap"),
                    dbc.Tab(label="Winrate Trend", tab_id="tab-trend", children=[
                        dbc.Label("Filter by Hero (optional):"),
                        dcc.Dropdown(id="hero-filter-dropdown", placeholder="No hero selected", className="mb-3"),
                        dcc.Graph(id="winrate-over-time"),
                    ]),
                    dbc.Tab(label="Match History", tab_id="tab-history", children=[
                        dbc.Row([
                            dbc.Col(width=6),
                            dbc.Col(dcc.Dropdown(id="history-load-amount-dropdown",
                                options=[
                                    {"label": "Load 10 more", "value": 10},
                                    {"label": "Load 25 more", "value": 25},
                                    {"label": "Load 50 more", "value": 50},
                                ], value=10, clearable=False), width=3),
                            dbc.Col(dbc.Button("Load More", id="load-more-history-button", color="secondary", className="w-100"), width=3),
                        ], className="my-3"),
                        html.Div(id="history-list-container", style={"maxHeight": "1000px", "overflowY": "auto"}),
                    ]),
                ], id="tabs", active_tab="tab-map")
            ], width=9),
        ]),
        dbc.Row(dbc.Col(dbc.Card([dbc.CardHeader(id="stats-header", className="bg-primary text-white"), dbc.CardBody([html.Div(id="stats-container")])]), width=12), className="mt-4"),
...
def get_map_image_url(map_name):
    if not isinstance(map_name, str): return "/assets/maps/default.png"
    cleaned_name = map_name.lower().replace(" ", "_").replace("'", "")
    for ext in [".jpg", ".png"]:
        asset_path = f"/assets/maps/{cleaned_name}{ext}"
        if os.path.exists(os.path.join("assets", "maps", f"{cleaned_name}{ext}")): return asset_path
    return "/assets/maps/default.png"

def get_hero_image_url(hero_name):
    if not isinstance(hero_name, str): return "/assets/heroes/default_hero.png"
    base_name = hero_name.lower()
    potential_names = list(set([base_name.replace(".", "").replace(":", "").replace("ú", "u").replace(" ", "_"), base_name.replace(".", "").replace(":", "").replace("ú", "u").replace(" ", ""), re.sub(r"[^a-z0-9]", "", base_name)]))
...
def filter_data(player, season=None, month=None, year=None):
    ...
    temp = df[df["Result"].isin(["VICTORY", "DEFEAT"])].copy()
    if season: temp = temp[temp["Season"] == season]
    else:
        if year is not None: temp = temp[pd.to_numeric(temp["Year"], errors="coerce") == int(year)]
        if month is not None: temp = temp[temp["Month"] == month]
    hero_col, role_col = f"{player} Hero", f"{player} Role"
    if hero_col not in temp.columns or role_col not in temp.columns: return pd.DataFrame()
...
def calculate_winrate(data, group_col):
    if data.empty or group_col not in data.columns: return pd.DataFrame(columns=[group_col, "Win", "Lose", "Winrate", "Games"])
    data[group_col] = data[group_col].astype(str).str.strip()
    data = data[data[group_col].notna() & (data[group_col] != "")]
    if data.empty: return pd.DataFrame(columns=[group_col, "Win", "Lose", "Winrate", "Games"])
...
def generate_history_layout_simple(games_df):
    ...
    for idx, game in games_df.iterrows():
        ...
        result_color, result_text = ("success", "VICTORY") if game.get("Result") == "VICTORY" else ("danger", "DEFEAT")
        player_list_items = []
        for p in config["known_players"]:
            ...
            if pd.notna(hero) and hero != "Not in game":
                ...
                player_list_items.append(dbc.ListGroupItem(html.Div([html.Img(src=hero_image_url, style={"width": "40px", "height": "40px", "borderRadius": "50%", "objectFit": "cover", "marginRight": "15px"}), html.Div([html.Div([html.Span(p, className="fw-bold"), html.Span(f" ({role})", className="text-muted", style={"fontSize": "0.9em"})]), html.Div(hero)], className="d-flex justify-content-between align-items-center w-100")], className="d-flex align-items-center")))
        history_items.append(dbc.Card(dbc.Row([dbc.Col(html.Img(src=map_image_url, className="img-fluid rounded-start h-100", style={"objectFit": "cover"}), md=3), dbc.Col([dbc.CardHeader(html.Div([html.Div([html.H5(f"{map_name}", className="mb-0"), html.Small(f"{gamemode} • {date_str}", className="text-muted")]), dbc.Badge(result_text, color=result_color, className="ms-auto", style={"height": "fit-content"})], className="d-flex justify-content-between align-items-center")), dbc.CardBody(dbc.ListGroup(player_list_items, flush=True), className="p-0")], md=9)], className="g-0"), className="mb-3"))
    return history_items

# --- Callbacks ---
@app.callback(Output("dummy-output", "children"), Input("update-data-button", "n_clicks"), prevent_initial_call=True)
def update_data(n_clicks):
    if n_clicks > 0: load_data(use_local=False)
    return f"Data updated at {pd.Timestamp.now()}"

@app.callback(Output("season-dropdown", "options"), Output("month-dropdown", "options"), Output("year-dropdown", "options"), Input("dummy-output", "children"))
...
@app.callback(Output("min-games-slider", "disabled"), Output("slider-hint", "children"), [Input("tabs", "active_tab"), Input("hero-stat-type", "value"), Input("role-stat-type", "value"), Input("map-stat-type", "value")])
def toggle_slider(tab, hero_stat, role_stat, map_stat):
    if (tab == "tab-hero" and hero_stat == "winrate") or (tab == "tab-role" and role_stat == "winrate") or (tab == "tab-map" and map_stat in ["winrate", "gamemode", "attackdef"]): return False, ""
    return True, "Only relevant for winrate statistics"

@app.callback(Output("history-list-container", "children"), Output("history-display-count-store", "data"), [Input("load-more-history-button", "n_clicks"), Input("dummy-output", "children")], [State("history-display-count-store", "data"), State("history-load-amount-dropdown", "value")])
def update_history_display(n_clicks, _, current_store, load_amount):
    global df
    if df.empty: return [dbc.Alert("No match history available.", color="danger")], {"count": 10}
...
def update_all_graphs(player, min_games, season, month, year, hero_filter, hero_stat_type, role_stat_type, map_stat_type, map_view_type, compare_values, compare_ids, _):
        ...
        for name, df_to_plot in dataframes.items():
            if not df_to_plot.empty and group_col and group_col in df_to_plot.columns:
                if y_col == "Winrate":
                    ...
                    if not stats.empty: bar_fig.add_trace(go.Bar(x=stats[group_col], y=stats[y_col], name=name, customdata=stats[["Games"]], hovertemplate="<b>%{x}</b><br>Winrate: %{y:.1%}<br>Games: %{customdata[0]}<extra></extra>"))
                else:
                    stats = df_to_plot.groupby(group_col).size().reset_index(name="Games").sort_values("Games", ascending=False)
                    if not stats.empty: bar_fig.add_trace(go.Bar(x=stats[group_col], y=stats[y_col], name=name, hovertemplate="<b>%{x}</b><br>Games: %{y}<extra></extra>"))
    ...
    if map_stat_type == "winrate": map_stat_output = dbc.Row(dbc.Col(dcc.Graph(figure=bar_fig), width=12))
    else:
        pie_fig = go.Figure()
        pie_data_col = None
        if map_stat_type == "gamemode": pie_data_col = "Gamemode"
        elif map_stat_type == "attackdef": pie_data_col = "Team 1 Side"
        if pie_data_col:
        ...
        for name, df_to_plot in dataframes.items():
            if not df_to_plot.empty:
                if y_col == "Winrate":
                    stats = calculate_winrate(df_to_plot, group_col)
                    stats = stats[stats["Games"] >= min_games]
                    if not stats.empty: fig.add_trace(go.Bar(x=stats[group_col], y=stats[y_col], name=name, customdata=stats[["Games"]], hovertemplate="<b>%{x}</b><br>Winrate: %{y:.1%}<br>Games: %{customdata[0]}<extra></extra>"))
                else:
                    stats = df_to_plot.groupby(group_col).size().reset_index(name="Games").sort_values("Games", ascending=False)
                    if not stats.empty: fig.add_trace(go.Bar(x=stats[group_col], y=stats[y_col], name=name, hovertemplate="<b>%{x}</b><br>Games: %{y}<extra></extra>"))
    ...
    for name, df_plot in dataframes.items():
        if not df_plot.empty and "Date" in df_plot.columns:
            ...
            if hero_filter: time_data = time_data[time_data["Hero"] == hero_filter]
...
if __name__ == "__main__":
    if not config: print("FATAL: config.json not found. Please run the setup script in the root directory of the project first.")
